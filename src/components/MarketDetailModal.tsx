...
import { Share2, ExternalLink, TrendingUp, Clock, X, Zap, Wallet } from 'lucide-react';
...
export function MarketDetailModal({
  isOpen, onClose, market, onBet,
}: {
  isOpen: boolean; onClose: () => void; market: Market | null; onBet?: (outcome: string, price: number) => void;
}) {
  ...
  useEffect(() => {
    if (isOpen && market) trackView(market.condition_id, market.question, 'modal', address);
  ...
  const getOutcomeColor = (idx: number) => {
    if (!isMultiOutcome) return idx === 0 ? '#00D26A' : '#FF4560';
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        ...
        className={[
          ...
          "w-full max-w-full h-full min-h-full rounded-none",
      ...
      >
        ...
        <div className="flex items-start gap-3 px-4 sm:px-6 py-3.5 border-b border-white/[0.06] bg-[#0A0908]/95 backdrop-blur sticky top-0 z-20 shrink-0">
          <div className="w-10 h-10 shrink-0 rounded-xl bg-white/[0.06] border border-white/[0.08] flex items-center justify-center overflow-hidden">
            {market.icon ? <img src={market.icon} alt="" className="w-full h-full object-cover" /> : <span className="text-base">🌍</span>}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-[11px] text-[#7A7068]">
              ...
              <span className="flex items-center gap-1 text-[#00D26A]"><Zap size={10} fill="#00D26A" />Injective EVM</span>
            </div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            <a href={`${EXPLORER_URL}/address/${market.marketAddress}`}
            ...
            <div className="flex-1 min-w-0 px-4 sm:px-6 pt-5 pb-6 flex flex-col gap-5">
                  ...
                  {outcomes.map((name: string, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center bg-white/[0.03] border border-white/[0.06] rounded-xl p-3 gap-3 hover:border-white/[0.12] transition-colors">
                        ...
                        <button onClick={() => setSelectedOutcome(name)}
              ...
              {market.description && (
                <div className="pt-3 border-t border-white/[0.05]">
                  ...
                  <p className="text-[12px] text-[#7A7068] leading-relaxed">{market.description}</p>
                </div>
              )}
            </div>
              ...
              <div className="lg:sticky lg:top-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[10px] text-[#00D26A] flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-[#00D26A] rounded-full animate-pulse" /> Injective EVM
                  </span>
                </div>
                ...
                <div className={`grid gap-2 mb-4 ${isMultiOutcome ? 'grid-cols-1' : 'grid-cols-2'}`}>
                  {outcomes.map((name: string, i: number) => {
                    ...
                    const color = getOutcomeColor(i);
                    return (
                      <button key={i} onClick={() => setSelectedOutcome(name)}
                        className={`cursor-pointer rounded-xl border font-bold transition-all hover:opacity-90 ${
                          isMultiOutcome ? 'p-2.5 flex items-center justify-between' : 'p-3 flex flex-col items-center justify-center gap-1'
                        } ${isSelected ? 'ring-2' : ''}`}
                        style={{
                    ...
                    {[10, 25, 50, 100].map(p => (
                      <button key={p} onClick={() => setOrderAmount(p)}
                        className={`cursor-pointer py-1.5 rounded-lg text-[11px] font-bold border transition-all ${
                          validAmount === p ? 'bg-white/[0.12] text-white border-white/[0.2]' : 'border-white/[0.07] bg-white/[0.04] text-[#7A7068] hover:text-white hover:bg-white/[0.09]'
                        }`}>${p}</button>
                    ))}
                  </div>
                </div>
                ...
                {(() => {
                  const selIdx = Math.max(0, outcomes.indexOf(selectedOutcome));
                  ...
                  const accentClr = getOutcomeColor(selIdx);
                  return (
                    <>
                      <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] divide-y divide-white/[0.05] mt-3">
                      ...
                      {!address ? (
                        <button onClick={() => toastError('Wallet required', 'Connect your wallet to place an order')}
                          className="cursor-pointer w-full py-3.5 rounded-xl font-bold text-[14px] flex items-center justify-center gap-2 bg-white/[0.06] border border-white/[0.1] text-[#7A7068] hover:text-white hover:bg-white/[0.1] transition-all mt-3">
                        ...
                        <button
                          ...
                          className="cursor-pointer w-full py-3.5 rounded-xl font-bold text-[14px] transition-all active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-3"
                ...
                <div className="flex items-center justify-center gap-3 mt-4">
                  <a href={`${EXPLORER_URL}/address/${market.marketAddress}`}
                    ...
                    <ExternalLink size={11} /> View on Injective
                  </a>
